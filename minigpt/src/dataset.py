...
class CharCorruptionDataset(Dataset):
    def __init__(self, data, block_size):
        ...
        # PAD_CHAR must be ID 0, MASK_CHAR must be ID 1
        ...
        # Split on newline; each line is treated as one document
    ...
    def __getitem__(self, idx):
        doc = self.data[idx]
        # Choose a random truncation length truncation_len
        max_trunc = int(self.block_size * 7 / 8)
        # Ensure at least 4 and at most len(doc) or max_trunc, whichever is smaller
        truncation_len = random.randint(4, max(4, min(len(doc), max_trunc)))
        truncated = doc[:truncation_len]
        # Choose a masked span length M in [1, floor(truncation_len/2)]
        #   and start index i in [0, truncation_len - M]
        # This yields avg(M)=~truncation_len/4
        M = random.randint(1, max(1, truncation_len // 2))
        i = random.randint(0, truncation_len - M)
        prefix = truncated[:i]
        masked_content = truncated[i:i + M]
        suffix = truncated[i + M:]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        # Pad to exactly block_size using PAD_CHAR on the right
        pad_len = self.block_size - len(masked_string)
        if pad_len > 0:
            masked_string = masked_string + (self.PAD_CHAR * pad_len)
        else:
            # pretty unlikely event where masked_string >= block_size
            masked_string = masked_string[: self.block_size]
        x_str = masked_string[:-1]
        y_str = masked_string[1:]
        # Convert each character to index
        x_ids = torch.tensor([self.stoi[c] for c in x_str], dtype=torch.long)
        y_ids = torch.tensor([self.stoi[c] for c in y_str], dtype=torch.long)
        return x_ids, y_ids
